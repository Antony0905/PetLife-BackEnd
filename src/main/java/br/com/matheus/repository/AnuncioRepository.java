...
public interface AnuncioRepository extends JpaRepository<Anuncio, Integer> {
	...
	@Query(value = "select a.* from anuncio a inner join usuario u on a.user_id = u.id where u.cidade = :cidade and a.titulo = :servico", nativeQuery = true)
	List<Anuncio> findAnuncioByCityAndService(@Param("cidade") String cidade, @Param("servico") String servico);

	@Query(value = "select a.* from anuncio a inner join usuario u on a.user_id = u.id where u.cidade = :cidade", nativeQuery = true)
	List<Anuncio> findAnuncioByCity(@Param("cidade") String cidade);

	@Query(value = "select * from anuncio where titulo = :servico", nativeQuery = true)
